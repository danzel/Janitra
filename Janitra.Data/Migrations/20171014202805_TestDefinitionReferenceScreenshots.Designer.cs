// <auto-generated />
using Janitra.Data;
using Janitra.Data.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage;
using Microsoft.EntityFrameworkCore.Storage.Internal;
using System;

namespace Janitra.Data.Migrations
{
    [DbContext(typeof(JanitraContext))]
    [Migration("20171014202805_TestDefinitionReferenceScreenshots")]
    partial class TestDefinitionReferenceScreenshots
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.0.0-rtm-26452")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Janitra.Data.Models.CitraBuild", b =>
                {
                    b.Property<int>("CitraBuildId")
                        .ValueGeneratedOnAdd();

                    b.Property<bool>("ActivelyTesting");

                    b.Property<int>("AddedByUserId");

                    b.Property<string>("BuildNotes")
                        .IsRequired();

                    b.Property<int>("BuildType");

                    b.Property<DateTimeOffset>("CommitTime");

                    b.Property<DateTimeOffset>("DateAdded");

                    b.Property<string>("GitHash")
                        .IsRequired()
                        .HasMaxLength(40);

                    b.Property<string>("LinuxUrl");

                    b.Property<string>("OsxUrl");

                    b.Property<string>("WindowsUrl");

                    b.HasKey("CitraBuildId");

                    b.HasIndex("AddedByUserId");

                    b.HasIndex("GitHash")
                        .IsUnique();

                    b.ToTable("CitraBuilds");
                });

            modelBuilder.Entity("Janitra.Data.Models.JanitraBot", b =>
                {
                    b.Property<int>("JanitraBotId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AccessKey")
                        .IsRequired();

                    b.Property<int>("AddedByUserId");

                    b.Property<string>("HardwareDetails")
                        .IsRequired();

                    b.Property<string>("Name")
                        .IsRequired();

                    b.Property<int>("Os");

                    b.Property<bool>("RunsHwTests");

                    b.HasKey("JanitraBotId");

                    b.HasIndex("AddedByUserId");

                    b.ToTable("JanitraBots");
                });

            modelBuilder.Entity("Janitra.Data.Models.Rom", b =>
                {
                    b.Property<int>("RomId")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("AddedByUserId");

                    b.Property<string>("Name")
                        .IsRequired();

                    b.Property<string>("RomFileName")
                        .IsRequired();

                    b.Property<string>("RomSha256")
                        .IsRequired()
                        .HasMaxLength(64);

                    b.Property<int>("RomType");

                    b.HasKey("RomId");

                    b.HasIndex("AddedByUserId");

                    b.ToTable("Roms");
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovie", b =>
                {
                    b.Property<int>("RomMovieId")
                        .ValueGeneratedOnAdd();

                    b.Property<bool>("ActivelyTesting");

                    b.Property<DateTimeOffset>("AddedAt");

                    b.Property<int>("AddedByUserId");

                    b.Property<int>("CitraRegionValue");

                    b.Property<string>("Description")
                        .IsRequired();

                    b.Property<TimeSpan>("Length");

                    b.Property<string>("MovieSha256")
                        .IsRequired()
                        .HasMaxLength(64);

                    b.Property<string>("MovieUrl")
                        .IsRequired();

                    b.Property<string>("Name")
                        .IsRequired();

                    b.Property<int>("RomId");

                    b.HasKey("RomMovieId");

                    b.HasIndex("AddedByUserId");

                    b.HasIndex("RomId");

                    b.ToTable("RomMovies");
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovieResult", b =>
                {
                    b.Property<int>("RomMovieResultId")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("CitraBuildId");

                    b.Property<int>("ExecutionResult");

                    b.Property<int>("JanitraBotId");

                    b.Property<string>("LogUrl")
                        .IsRequired();

                    b.Property<DateTimeOffset>("ReportedAt");

                    b.Property<int>("RomMovieId");

                    b.Property<TimeSpan>("TimeTaken");

                    b.HasKey("RomMovieResultId");

                    b.HasIndex("CitraBuildId");

                    b.HasIndex("JanitraBotId");

                    b.HasIndex("RomMovieId");

                    b.ToTable("RomMovieResults");
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovieResultScreenshot", b =>
                {
                    b.Property<int>("RomMovieResultScreenshotId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("BottomImageUrl")
                        .IsRequired();

                    b.Property<int>("FrameNumber");

                    b.Property<int>("RomMovieResultId");

                    b.Property<string>("TopImageUrl")
                        .IsRequired();

                    b.HasKey("RomMovieResultScreenshotId");

                    b.HasIndex("RomMovieResultId");

                    b.ToTable("RomMovieResultScreenshot");
                });

            modelBuilder.Entity("Janitra.Data.Models.TestDefinition", b =>
                {
                    b.Property<int>("TestDefinitionId")
                        .ValueGeneratedOnAdd();

                    b.Property<bool>("ActivelyTesting");

                    b.Property<DateTimeOffset>("AddedAt");

                    b.Property<int>("AddedByUserId");

                    b.Property<string>("MovieSha256")
                        .IsRequired()
                        .HasMaxLength(64);

                    b.Property<string>("MovieUrl")
                        .IsRequired();

                    b.Property<string>("Notes")
                        .IsRequired();

                    b.Property<string>("ReferenceScreenshotBottomUrl")
                        .IsRequired();

                    b.Property<string>("ReferenceScreenshotTopUrl")
                        .IsRequired();

                    b.Property<string>("TestName")
                        .IsRequired();

                    b.Property<int>("TestRomId");

                    b.HasKey("TestDefinitionId");

                    b.HasIndex("AddedByUserId");

                    b.HasIndex("TestName")
                        .IsUnique();

                    b.HasIndex("TestRomId");

                    b.ToTable("TestDefinitions");
                });

            modelBuilder.Entity("Janitra.Data.Models.TestResult", b =>
                {
                    b.Property<int>("TestResultId")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("AccuracyStatus");

                    b.Property<int>("CitraBuildId");

                    b.Property<int>("ExecutionResult");

                    b.Property<int>("JanitraBotId");

                    b.Property<string>("LogUrl")
                        .IsRequired();

                    b.Property<DateTimeOffset>("ReportedAt");

                    b.Property<string>("ScreenshotBottomUrl")
                        .IsRequired();

                    b.Property<string>("ScreenshotTopUrl")
                        .IsRequired();

                    b.Property<int>("TestDefinitionId");

                    b.Property<TimeSpan>("TimeTaken");

                    b.HasKey("TestResultId");

                    b.HasIndex("JanitraBotId");

                    b.HasIndex("TestDefinitionId");

                    b.HasIndex("CitraBuildId", "TestDefinitionId", "JanitraBotId")
                        .IsUnique();

                    b.ToTable("TestResults");
                });

            modelBuilder.Entity("Janitra.Data.Models.TestRom", b =>
                {
                    b.Property<int>("TestRomId")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTimeOffset>("AddedAt");

                    b.Property<int>("AddedByUserId");

                    b.Property<string>("CodeUrl")
                        .IsRequired();

                    b.Property<string>("RomSha256")
                        .IsRequired()
                        .HasMaxLength(64);

                    b.Property<string>("RomUrl")
                        .IsRequired();

                    b.HasKey("TestRomId");

                    b.HasIndex("AddedByUserId");

                    b.ToTable("TestRoms");
                });

            modelBuilder.Entity("Janitra.Data.Models.User", b =>
                {
                    b.Property<int>("UserId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("OAuthId")
                        .IsRequired();

                    b.Property<string>("OAuthName")
                        .IsRequired();

                    b.Property<string>("OAuthProvider")
                        .IsRequired();

                    b.Property<int>("UserLevel");

                    b.HasKey("UserId");

                    b.HasIndex("OAuthProvider", "OAuthId")
                        .IsUnique();

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Janitra.Data.Models.CitraBuild", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedCitraBuilds")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.JanitraBot", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedJanitraBots")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.Rom", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedRoms")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovie", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedRomMovies")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Janitra.Data.Models.Rom", "Rom")
                        .WithMany("Movies")
                        .HasForeignKey("RomId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovieResult", b =>
                {
                    b.HasOne("Janitra.Data.Models.CitraBuild", "CitraBuild")
                        .WithMany("RomMovieResults")
                        .HasForeignKey("CitraBuildId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Janitra.Data.Models.JanitraBot", "JanitraBot")
                        .WithMany("RomMovieResults")
                        .HasForeignKey("JanitraBotId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Janitra.Data.Models.RomMovie", "RomMovie")
                        .WithMany("Results")
                        .HasForeignKey("RomMovieId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.RomMovieResultScreenshot", b =>
                {
                    b.HasOne("Janitra.Data.Models.RomMovieResult", "RomMovieResult")
                        .WithMany("Screenshots")
                        .HasForeignKey("RomMovieResultId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Janitra.Data.Models.TestDefinition", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedTestDefinitions")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Janitra.Data.Models.TestRom", "TestRom")
                        .WithMany("UsedByTestDefinitions")
                        .HasForeignKey("TestRomId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Janitra.Data.Models.TestResult", b =>
                {
                    b.HasOne("Janitra.Data.Models.CitraBuild", "CitraBuild")
                        .WithMany("TestResults")
                        .HasForeignKey("CitraBuildId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Janitra.Data.Models.JanitraBot", "JanitraBot")
                        .WithMany("TestResults")
                        .HasForeignKey("JanitraBotId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Janitra.Data.Models.TestDefinition", "TestDefinition")
                        .WithMany("TestResults")
                        .HasForeignKey("TestDefinitionId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Janitra.Data.Models.TestRom", b =>
                {
                    b.HasOne("Janitra.Data.Models.User", "AddedByUser")
                        .WithMany("AddedTestRoms")
                        .HasForeignKey("AddedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);
                });
#pragma warning restore 612, 618
        }
    }
}
